/*
  ...
  @[December 30, 2025] > Changed name from "old" StMuFcsPi0TreeMaker to the "new" StMuFcsTreeMaker which will serve as the base class for similar FCS point analysis. Changed many members to static so that they are shared among the different instances of the makers if running different makers in parallel.
  @[February 11, 2026] > This is now an obsolete and incomplete class for FCS analysis. The idea was to create a base class that other "analyzers" could inherit from and reuse some of its members. After extensive experimentation and information from Google Gemini I realized it is better to separate the "analyzers" from the "data". This is known as the "strategy" design pattern and STAR uses a similar idea with their "StMakers". The new framework was implemented in a different folder called StMuFcsAna. I will keep this here as a learning tool and as a repository of the history of the analysis framework. It also contains some useful links for c++ coding.
*/


#ifndef STMUFCSTREEMAKER_HH
#define STMUFCSTREEMAKER_HH
...
class StMuFcsTreeMaker : public StMaker {
public:
  
  StMuFcsTreeMaker(const Char_t* name = "MuFcsTreeMaker");
  ~StMuFcsTreeMaker();
  virtual Int_t Init();
  ...
  Int_t Make_GetEpdColl();     ///< (Get EPD collection and/or hits) Function that will set the variables related to grabbing the EPD collection. This is needed if you are not sure if the EPD collection is coming from MuDst data or the trigger data
  Int_t Make_VertexInfo();         ///< Get and set members related to the collision vertex
  Int_t Make_FillFcsClusPoint();   ///< Get and set the clusters and points to #mPhArr
  virtual Int_t Make_CheckAndSetEpdHit() = 0;  ///< Loop over #mPhArr and check which photon candidates pair with which EPD tiles
  virtual Int_t Make_PointPairs(TClonesArray* pointpairs) = 0;     ///< Loop over all the photon (point) candidates and make photon pairs with and save them to the TClonesArray that will hold the particle candidates
  virtual Int_t Make_TssaAna(TClonesArray* pointpairs) = 0;        ///< Loop over the point pairs and apply some cut criteria to then make histograms for the Transverse Single Spin Asymmetry (TSSA) analysis
  ...
  virtual void Print(Option_t* opt="") const; //"e" for event, "t" for trigger, "g" for photon, "p" for photon pair, "a" for all
  ...
  */
  ...
  static void DoTssaAna( TH1* npi0[][2], TH1* h1_rawasymVphi[][StMuFcsTreeMaker::NXFBIN] );    ///< Compute asymmetry from npi0 array of [blue,yellow][spin up,down] and return an array of raw assymtries by [blue,yellow][energybin]
  static void DoTssaFit(TH1* h1_rawasymVphi[][StMuFcsTreeMaker::NXFBIN], TH1* h1_bluepoldata, TH1* h1_yellowpoldata, TH1* h1_AnResult[]);
...
protected:
  static StMuDstMaker* mMuDstMkr;
  static StMuDst* mMuDst;
  static StMuEvent* mMuEvent;
  static const StTriggerData* mTrigData;
  static StRunInfo* mRunInfo;
  static StSpinDbMaker* mSpinDbMkr;

  static StFcsDb* mFcsDb;
  static StMuFcsCollection* mMuFcsColl;
  static StFcsRun22TriggerMap* mFcsTrigMap;

  static StEpdGeom* mEpdGeo;
  static TClonesArray* mMuEpdHits;
  static StEpdHitMaker* mEpdHitMkr;
  static StEpdCollection* mEpdColl;
  static StMuEpdRun22QaMaker* mEpdQaMkr;
  
  static std::vector<std::string> mTargetTrig;  ///< For Target Trigger ID
  static bool mIgnoreTrig;                      ///< (false) flag to check if ignoring triggers or not
  ...
  static TTree* mPi0Tree;                  ///< #TTree with desired branches
  static FcsEventInfo* mEvtInfo;           ///< #FcsEventInfo object for TTree
  //For trigger branch of tree
  static const UShort_t mMaxTrigs = 65;    ///< 64 FCS triggers + 1 for any other not found
  static Int_t mNTrig;                     ///< Total triggers in the event
  static Int_t mTriggers[mMaxTrigs];       ///< Array of Triggers in the event
  static TClonesArray* mPhArr;             ///< #TClonesArray of all #FcsPhotonCandidate
  static TClonesArray* mPhPairArr;         ///< Array of #FcsPhotonCandidate pairs to be used later in analysis
  ...
  static bool mValidTrigFound;       ///< (false) Found a trigger that matches the ones specified or #mIgnoreTrig is on
  static bool mEmTrigFound;          ///< (false) Found a trigger that mathces one of the FCS EM triggers
  ...
  static short mTrigEm0;            ///< Gets set to 1 if EM0 or EM0_tpc trigger was fired
  static short mTrigEm1;            ///< Gets set to 2 if EM1 or EM1_tpc trigger was fired
  static short mTrigEm2;            ///< Gets set to 3 if EM2 or EM2_tpc trigger was fired
  static short mTrigEm3;            ///< Gets set to 4 if EM3 or EM3_tpc trigger was fired

  static short mFoundVertex;              ///< (0) Bit vector for knowing which vertex was used 0 means no vertex, 1=Vpd,2=Epd,4=Bbc
  static Double_t mUseVertex;             ///< (-999.0) Vertex to use in analysis
  static Double_t mVertexCutLow;          ///< (-150.0) Variables for z vertex cuts at low end in cm
  static Double_t mVertexCutHigh;         ///< (150.0) Variables for z vertex cuts at high end in cm
  
  static Double_t mEnCut;                 ///< (1.0) Energy Cut for #FcsPhotonCandidates
  static Double_t mEpdNmipCut;            ///< (0.4) Cut on EPD nmip to classify cluster or point as charged or uncharged
  static UShort_t mTreeOnBitMap;          ///< (0x7) Turn on or off branches in the pi0 tree. first bit is events, second bit is photon branch, third bit is pi0 branch. Turn on all branches by default
  ...
  static std::map<Int_t,PolData*> mPolarizationData;   ///< Map of polarization data from file with fill number as key to quickly look up from data structure
  ...
  static std::map<Int_t,TPolyLine*> mEpdTileMap;    ///< EPD "tile key" to polyline for drawing
...
private:  
  ...
  static TRandom3 mSpinRndm;                 ///< Spin state randomizer

  static bool mMakeEvent;
  static bool mMakePolarization;
  static bool mMakeTrigger;
  static bool mMakeSpin;
  static bool mMakeEpdColl;  
  static bool mMakeVertex;
  static bool mMakeFillClusPoint;
  //static bool mMakeDoEpdHit = false;
  //static bool mMakePointPairs = false;
  //static bool mMakeTssaAna = false;

  ClassDef(StMuFcsTreeMaker, 1)
};
