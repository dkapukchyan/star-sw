...
#include "StMuFcsPi0Maker.h"

#ifndef SKIPDefImp
ClassImp(StMuFcsPi0Maker)
#endif


StMuFcsPi0Maker::StMuFcsPi0Maker(const Char_t* name) : StMaker(name)
{}

StMuFcsPi0Maker::~StMuFcsPi0Maker()
{}

Int_t StMuFcsPi0Maker::InitRun(int runnumber) {
...
//-----------------------
Int_t StMuFcsPi0Maker::Finish() {
  if( mFilename.Length() == 0){ return kStOk; }
  TFile MyFile(mFilename.Data(), "RECREATE");
...
//----------------------
Int_t StMuFcsPi0Maker::Make() {

  mMuDstMkr = (StMuDstMaker*)GetInputDS("MuDst");
  if( mMuDstMkr==0 ){ LOG_ERROR <<"StMuFcsPi0Maker::Make - !MuDstMkr" <<endm; return kStErr; }
  mMuDst = mMuDstMkr->muDst();
  if( mMuDst==0 ){ LOG_ERROR << "StMuFcsPi0Maker::Make - !MuDst" << endm; return kStErr; }
  mMuEvent = mMuDst->event();
  if( mMuEvent==0 ){ LOG_ERROR <<"StMuFcsPi0Maker::Make - !MuEvent" <<endm; return kStErr; }
  mTrigData = mMuEvent->triggerData();
  if( mTrigData==0 ){ LOG_ERROR <<"StMuFcsPi0Maker::Make - !TrigData" <<endm; return kStErr; }
  mRunInfo = &(mMuEvent->runInfo());
  if( mRunInfo==0 ){ LOG_ERROR <<"StMuFcsPi0Maker::Make - !RunInfo" <<endm; return kStErr; }

  mMuFcsColl = mMuDst->muFcsCollection();
  if (!mMuFcsColl) { LOG_ERROR << "StMuFcsPi0Maker::Make did not find MuFcsCollection" << endm; return kStErr; }

  h1_num_entries->Fill(1);
  if (mNAccepted < mMaxEvents) {
    mNEvents++;
    //cout << "current event:" << mNEvents << endl;
    if (mFilter == 1 && mMuFcsColl->numberOfHits(0) + mMuFcsColl->numberOfHits(1) + mMuFcsColl->numberOfHits(2) + mMuFcsColl->numberOfHits(3) == 0){ return kStOK; }
	...
	StMuFcsHit* hit = (StMuFcsHit*)hits->At(i);
	...
	n_EcalMult++;
      ...
      for (int i = mMuFcsColl->indexOfFirstCluster(det); i < nc; i++) {
	StMuFcsCluster* clu = (StMuFcsCluster*)clusters->At(i);
	...
	n_EcalClustMult++;
	if( clu_energy > E_min){ n_EcalClust_cut++; }
	if( clu->energy() >= E_min ){ h2_cluster_position_cut->Fill(xyz.x(), xyz.y()); }
