...
bool StMuFcsTreeMaker::mMakeEvent = false;
bool StMuFcsTreeMaker::mMakePolarization = false;
bool StMuFcsTreeMaker::mMakeTrigger = false;
bool StMuFcsTreeMaker::mMakeSpin = false;
bool StMuFcsTreeMaker::mMakeEpdColl = false;  
bool StMuFcsTreeMaker::mMakeVertex = false;
bool StMuFcsTreeMaker::mMakeFillClusPoint = false;

//const double StMuFcsTreeMaker::xfbins[] = {0, 0.04, 0.08, 0.12, 0.16, 0.2, 0.24, 0.28, 0.32, 0.36, 0.4, 0.44, 0.48, 0.52};
const Double_t StMuFcsTreeMaker::xfbins[] = {0, 0.04, 0.08, 0.12, 0.16, 0.2, 0.24, 0.3, 0.5}; //@[August 4, 2025] > New xF binning

StMuDstMaker* StMuFcsTreeMaker::mMuDstMkr = 0;
StMuDst* StMuFcsTreeMaker::mMuDst = 0;
StMuEvent* StMuFcsTreeMaker::mMuEvent = 0;
StRunInfo* StMuFcsTreeMaker::mRunInfo = 0;
StSpinDbMaker* StMuFcsTreeMaker::mSpinDbMkr = 0;

StFcsDb* StMuFcsTreeMaker::mFcsDb = 0;
StMuFcsCollection* StMuFcsTreeMaker::mMuFcsColl = 0;
StFcsRun22TriggerMap* StMuFcsTreeMaker::mFcsTrigMap = 0;

StEpdGeom* StMuFcsTreeMaker::mEpdGeo = 0;
TClonesArray* StMuFcsTreeMaker::mMuEpdHits = 0;
StEpdHitMaker* StMuFcsTreeMaker::mEpdHitMkr = 0;
StEpdCollection* StMuFcsTreeMaker::mEpdColl = 0;
StMuEpdRun22QaMaker* StMuFcsTreeMaker::mEpdQaMkr = 0;
  
bool StMuFcsTreeMaker::mIgnoreTrig = false;

TTree* StMuFcsTreeMaker::mPi0Tree = 0;
FcsEventInfo* StMuFcsTreeMaker::mEvtInfo = 0;

Int_t StMuFcsTreeMaker::mNTrig = 0;
TClonesArray* StMuFcsTreeMaker::mPhArr = 0;
TClonesArray* StMuFcsTreeMaker::mPhPairArr = 0;

bool StMuFcsTreeMaker::mValidTrigFound = false;
bool StMuFcsTreeMaker::mEmTrigFound = false;

short StMuFcsTreeMaker::mTrigEm0 = -1;
short StMuFcsTreeMaker::mTrigEm1 = -1;
short StMuFcsTreeMaker::mTrigEm2 = -1;
short StMuFcsTreeMaker::mTrigEm3 = -1;

short StMuFcsTreeMaker::mFoundVertex = 0;
Double_t StMuFcsTreeMaker::mUseVertex = -999.0;
Double_t StMuFcsTreeMaker::mVertexCutLow = -150.0;
Double_t StMuFcsTreeMaker::mVertexCutHigh = 150.0;
  
Double_t StMuFcsTreeMaker::mEnCut = 1.0;
Double_t StMuFcsTreeMaker::mEpdNmipCut = 0.4;
UShort_t StMuFcsTreeMaker::mTreeOnBitMap = 0x7;

std::map<Int_t,TPolyLine*> StMuFcsTreeMaker::mEpdTileMap;
std::vector<std::string> StMuFcsTreeMaker::mTargetTrig;
TRandom3 StMuFcsTreeMaker::mSpinRndm;
std::map<Int_t,StMuFcsTreeMaker::PolData*> StMuFcsTreeMaker::mPolarizationData;
const StTriggerData* StMuFcsTreeMaker::mTrigData;
Int_t StMuFcsTreeMaker::mTriggers[mMaxTrigs];

StMuFcsTreeMaker::StMuFcsTreeMaker(const Char_t* name) : StMaker(name)
{
  mSpinRndm.SetSeed(0);
...
StMuFcsTreeMaker::~StMuFcsTreeMaker()
{
  delete mEvtInfo;
  delete mPhArr;
  delete mPhPairArr;
...
/*#ifndef __CINT__
void StMuFcsTreeMaker::SetTrigs(const char* trigname,...)
{
  va_list args;
...
void StMuFcsTreeMaker::setEventBit(bool val)
{
  if( val ){ mTreeOnBitMap |= 0x01; }
  else{ mTreeOnBitMap &= ~(0x01); }
}

void StMuFcsTreeMaker::setPhotonOn(bool val)
{
  if( val ){ mTreeOnBitMap |= 0x02; }
  else{ mTreeOnBitMap &= ~(0x02); }
}

void StMuFcsTreeMaker::setPhPairOn(bool val)
{
  if( val ){ mTreeOnBitMap |= 0x04; }
  else{ mTreeOnBitMap &= ~(0x04); }
}

/*bool StMuFcsTreeMaker::checkTreeOnBit(UShort_t bit)
{
  if( mTreeOnBitMap & (1<<bit) ){ return true; }
  else{ return false; }
  }*/

bool StMuFcsTreeMaker::isEventOn() const
{
  if( mTreeOnBitMap & 0x01 ){ return true; }
  else{ return false; }
}

bool StMuFcsTreeMaker::isPhotonOn() const
{
  if( mTreeOnBitMap & 0x02 ){ return true; }
  else{ return false; }
}

bool StMuFcsTreeMaker::isPhPairOn() const
{
  if( mTreeOnBitMap & 0x04 ){ return true; }
  else{ return false; }
}

void StMuFcsTreeMaker::setHistManager( HistManager* hm )
{
  if( mInternalHists ){ delete mHists; mHists = 0; }
  mInternalHists = false;
  if( mHists!=0 ){ LOG_WARN << "StMuFcsTreeMaker::setHistManager() - HistManager exists and is external - no changes made" << endm; return; }
  else{ mHists = hm; }
}

/*void StMuFcsTreeMaker::CopyHists(StFcsTreeMaker* othermkr)
{
  mH1D_Entries            = othermkr->mH1D_Entries;
  mH1D_BluePol            = othermkr->mH1D_BluePol;
  mH1D_YellowPol          = othermkr->mH1D_YellowPol;
  mH1D_BluePolErr         = othermkr->mH1D_BluePolErr;
  mH1D_YellowPolErr       = othermkr->mH1D_YellowPolErr;
  mH1F_Triggers           = othermkr->mH1F_Triggers;
  mH2F_foundVvertex       = othermkr->mH2F_foundVvertex;
  mH1F_RndmSpin           = othermkr->mH1F_RndmSpin;

  mH2F_PhotonHeatMap      = othermkr->mH2F_PhotonHeatMap;
  mH2F_PhotonHeatMapG     = othermkr->mH2F_PhotonHeatMapG;
  mH2F_PhotonHeatMapB     = othermkr->mH2F_PhotonHeatMapB;
  mH2F_EpdProjHitMap      = othermkr->mH2F_EpdProjHitMap;
  mH2F_EpdProjHitMap_Vcut = othermkr->mH2F_EpdProjHitMap_Vcut;
  mH2F_EpdNmip            = othermkr->mH2F_EpdNmip;

  mH1F_ClusterEnergy      = othermkr->mH1F_ClusterEnergy;
  mH1F_PointEnergy        = othermkr->mH1F_PointEnergy;
  mH2F_Energy_ph1Vph2     = othermkr->mH2F_Energy_ph1Vph2;
  mH1F_NBadEpdProj        = othermkr->mH1F_NBadEpdProj;
  mH1F_NBadEpdProjVcut    = othermkr->mH1F_NBadEpdProjVcut;

  mH1F_PointMult          = othermkr->mH1F_PointMult;
  }*/

UInt_t StMuFcsTreeMaker::LoadHists( TFile* file )
{
...
Int_t StMuFcsTreeMaker::Init()
{
  ...
  if( mHists==0 ){
    LOG_INFO << "StMuFcsTreeMaker::Init() - No HistManager specified. Creating a new one with file name " << mFilename << ". Potential conflicts exist if a HistManager exists with same file name" << endm;
  ...
  if( mFcsTrigMap==0 ){ LOG_WARN << "StMuFcsTreeMaker::Init() - No Trigger Map found" << endm; }
  else{ if( mFcsTrigMap->sizeOfTriggers()<=0 ){ LOG_WARN << "StMuFcsTreeMaker::Init() - Trigger Map is empty" << endm; } }
  ...
  LOG_INFO << "StMuFcsTreeMaker::Init() - Loaded " << totalhists << " histograms" << endm;

  return kStOk;
}

UInt_t StMuFcsTreeMaker::LoadDataFromFile(TFile* file) //, TTree&* tree, FcsEventInfo&* evt,Int_t& ntrig, Int_t&* triggers,  TClonesArray&* pharr, TClonesArray&* pi0arr, TH1&* hist)
{
  ...
  if( mFcsTrigMap==0 ){ std::cout << "StMuFcsTreeMaker::LoadDataFromFile() - No Trigger Map found" << std::endl; }
  else{ if( mFcsTrigMap->sizeOfTriggers()<=0 ){ std::cout << "StMuFcsTreeMaker::LoadDataFromFile() - Trigger Map is empty" << std::endl; } }
...
Int_t StMuFcsTreeMaker::InitRun(int runnumber)
{
  std::cout << this->ClassName()<<"|Start InitRun:|fcsdb:"<<mFcsDb << std::endl;
  if( mFcsDb==0 ){
    mFcsDb = static_cast<StFcsDb*>(GetDataSet("fcsDb"));
    //mFcsDb->setDbAccess(0);
    if(!mFcsDb){
      LOG_ERROR << "StMuFcsTreeMaker::InitRun - Failed to get StFcsDbMaker" << endm;
      return kStFatal;
    }
  }
  //std::cout << "End FcsDb:|fcsdb:"<<mFcsDb <<"|spindb:"<<mSpinDbMkr << std::endl;

  if( mSpinDbMkr==0 ){
    mSpinDbMkr = static_cast<StSpinDbMaker*>(GetMaker("spinDb"));
    if( !mSpinDbMkr ){
      LOG_WARN << "StMuFcsTreeMaker::InitRun - Could not find StSpinDbMaker named 'spinDb'" << endm;
    }
    else{
      if( !mSpinDbMkr->isValid() ){
	LOG_WARN << "StMuFcsTreeMaker::InitRun - Found StSpinDbMaker but contains invalid data so will not use it" << endm;
	mSpinDbMkr=0;
  ...
  //std::cout << "End Spin:|spindb:"<<mSpinDbMkr<<"|epdgo:"<<mEpdGeo << std::endl;
  if( mEpdGeo==0 ){ mEpdGeo = new StEpdGeom(); }
  /*else{
    LOG_INFO << "StMuFcsTreeMaker::InitRun - StEpdGeom Exists!" << endm;
    return kStFatal;
    }*/
  //std::cout << "End EpdGeo:|epdgeo:"<<mEpdGeo<<"|EpdQaMkr:"<<mEpdQaMkr << std::endl;
  if( mEpdQaMkr==0 ){
    mEpdQaMkr = (StMuEpdRun22QaMaker*) GetMaker("FcsEpdRun22Qa");
    if( mEpdQaMkr==0 ){
      LOG_WARN << "StMuFcsTreeMaker::InitRun - No StMuEpdRun22QaMaker found, EPD vertex information may be missing" << endm;
    }
  }
  //std::cout << "End InitRun:|EpdQaMkr:"<<mEpdQaMkr << std::endl;
  return kStOK;
}

//-----------------------
Int_t StMuFcsTreeMaker::Finish()
{
...
//----------------------
Int_t StMuFcsTreeMaker::Make()
{
  std::cout << this->ClassName() << "|Start Make" << std::endl;
  //Checks below are specific to the kind of return those functions will "normally" return
  Int_t result = kStOk;
  if( !mMakeEvent ){
    result = Make_LoadEvent();
    if( result==kStErr ){ return result; }
    mMakeEvent = true;
  }
  if( !mMakePolarization ){
    result = Make_Polarization();
    if( result == kStErr ){ return result; }
    mMakePolarization = true;
  }
  ...
  if( !mMakeEpdColl ){
    ...
    if( result==kStWarn ){ return result; }
  }
  ...
  result = this->Make_PointPairs(mPhPairArr);
  if( result!=kStOk ){ return result; }
  
  result = this->Make_TssaAna(mPhPairArr);
  if( result==kStSkip ){ return result; }
...
Int_t StMuFcsTreeMaker::Make_LoadEvent()
{
  if( mMakeEvent ){ return kStOk; }
  std::cout << this->ClassName() << "|Start:Make_LoadEvent()" << std::endl;
  mMuDstMkr = (StMuDstMaker*)GetInputDS("MuDst");
  if( mMuDstMkr==0 ){ LOG_ERROR <<"StMuFcsTreeMaker::Make_LoadEvent - !MuDstMkr" <<endm; return kStErr; }
  mMuDst = mMuDstMkr->muDst();
  if( mMuDst==0 ){ LOG_ERROR << "StMuFcsTreeMaker::Make_LoadEvent - !MuDst" << endm; return kStErr; }
  mMuEvent = mMuDst->event();
  if( mMuEvent==0 ){ LOG_ERROR <<"StMuFcsTreeMaker::Make_LoadEvent - !MuEvent" <<endm; return kStErr; }
  mTrigData = mMuEvent->triggerData();
  if( mTrigData==0 ){ LOG_ERROR <<"StMuFcsTreeMaker::Make_LoadEvent - !TrigData" <<endm; return kStErr; }
  mRunInfo = &(mMuEvent->runInfo());
  if( mRunInfo==0 ){ LOG_ERROR <<"StMuFcsTreeMaker::Make_LoadEvent - !RunInfo" <<endm; return kStErr; }
...
Int_t StMuFcsTreeMaker::Make_Polarization()
{
  if( mMakePolarization ){return kStOk; }
  std::cout << this->ClassName() << "|Start:Make_Polarization()" << std::endl;
  ...
  if( mPolarizationData.find(fillnum) == mPolarizationData.end() ){
    //If no polarization data for this fill then set it equal to zero so it doesn't contribute to the sum
    poldat = new PolData();
...
Int_t StMuFcsTreeMaker::Make_CheckAndSetTrig()
{
  if( mMakeTrigger ){ return kStOk; }
  std::cout << this->ClassName() << "|Start:Make_CheckAndSetTrig()" << std::endl;
  ...
  if( !TrigMuColl ){ LOG_ERROR <<"StMuFcsTreeMaker::FillEventInfo - !TrigMuColl" <<endl; return kStErr; }
...
Int_t StMuFcsTreeMaker::Make_SpinInfo()
{
  if( mMakeSpin ){ return kStOk; }
  std::cout << this->ClassName() << "|Start:Make_SpinInfo()" << std::endl;
  //Spin information
  if( mSpinDbMkr==0 ){
...
Int_t StMuFcsTreeMaker::Make_VertexInfo()
{
  std::cout << this->ClassName() << "|Start Make_VertexInfo|evt:"<<mEvtInfo <<"|trig:"<<mTrigData << std::endl;
  //Vertex Information
  mEvtInfo->mVpdVz = -999;
...
Int_t StMuFcsTreeMaker::Make_GetEpdColl()
{
  if( mMakeEpdColl ){ return kStOk; }
  std::cout << this->ClassName() << "|Start:Make_GetEpdColl()" << std::endl;
  ...
  if( mMuEpdHits==0 ){ LOG_INFO << "StMuFcsTreeMaker::Make - No MuEPD hits" << endm;
    mEpdHitMkr = (StEpdHitMaker*)GetMaker("epdHit");
    if( mEpdHitMkr==0 ){ LOG_WARN << "StMuFcsTreeMaker::Make - No StEpdHitMaker(\"epdHit\")" << endm; return kStWarn; }
...
Int_t StMuFcsTreeMaker::Make_FillFcsClusPoint()
{
  std::cout << this->ClassName() << "|Start:Make_FillFcsClusPoint()" << std::endl;
  //Fcs Collection
  mMuFcsColl = mMuDst->muFcsCollection();
  if (!mMuFcsColl) { LOG_ERROR << "StMuFcsTreeMaker::Make did not find MuFcsCollection" << endm; return kStErr; }
...
Int_t StMuFcsTreeMaker::Make_CheckAndSetEpdHit()
{
  ...
  else{ LOG_ERROR << "StMuFcsTreeMaker::FillEpdinfo() - If you see this error then there is a bug that is setting EPD hits improperly" << endm; return kStErr; }
  ...
  for( Int_t iph = 0; iph<ntotal; ++iph ){
    ...
    this->CheckInsideEpdTile(ph,epdproj.at(0),epdproj.at(1));  //Function that will check which EPD tiles photon candidate overlaps with and sets the appropriate variables for it
    /*
...
Int_t StMuFcsTreeMaker::Make_PointPairs(TClonesArray* pointpairs)
{
  std::cout << this->ClassName() << "|Start:Make_PointPairs()" << std::endl;
...
Int_t StMuFcsTreeMaker::Make_TssaAna(TClonesArray* pointpairs)
{
  return kStOk;
}

void StMuFcsTreeMaker::Clear(Option_t* option)
{
  //Reset Variables
  mMakeEvent         = false;
  mMakePolarization  = false;
  mMakeTrigger       = false;
  mMakeSpin          = false;
  mMakeEpdColl       = false;
  mMakeVertex        = false;
  mMakeFillClusPoint = false;
...
void StMuFcsTreeMaker::Print(Option_t* opt) const
{
  TString option(opt);
  option.ToLower();
...
std::vector<Double_t> StMuFcsTreeMaker::ProjectToEpd(Double_t xfcs, Double_t yfcs, Double_t zfcs, Double_t zvertex)
{
...
void StMuFcsTreeMaker::PaintEventQa(TCanvas* canv,  const char* savename) const
{
  canv->Clear();
...
void StMuFcsTreeMaker::PaintPolarization(TCanvas* canv, const char* savename) const
{
  canv->Clear();
...
void StMuFcsTreeMaker::PaintPhotonQa(TCanvas* canv, const char* savename)   const
{
  canv->Clear();
...
void StMuFcsTreeMaker::AddHistStatsOneline( TLegend* HistLeg, const TH1* h1, const std::string &title )
{
...
void StMuFcsTreeMaker::PaintEnergyZoom(TCanvas* canv, const char* savename) const
{
  canv->Clear();
...
void StMuFcsTreeMaker::PaintEpdNmipCuts(TCanvas* canv, const char* savename ) const
{
  canv->Clear();
  canv->Divide(2,1);
...
/*  
Int_t StMuFcsTreeMaker::LoadGraphsFromFile(TFile* file, TObjArray* graphs )
{
  Int_t gloaded = 0;
...
void StMuFcsTreeMaker::FillGraphs(Int_t irun)
{
...
void StMuFcsTreeMaker::DrawQaGraphs(TCanvas* canv, const char* savename)
{
  canv->Clear();
  canv->cd();
  canv->Divide(1,2);
...
*/
/*
void StMuFcsTreeMaker::MergeForTssa( TH1* totalhistinc[][2], TH1* totalhistbg1[][2], TH1* totalhistbg2[][2], TH3* mergedinvmass, TH1* mergedpolblue, TH1* mergedpolyell, TH1* mergedpolblueerr, TH1* mergedpolyellerr )
{
...
void StMuFcsTreeMaker::DoTssaAna( TH1* npi0[][2], TH1* h1_rawasymVphi[][StMuFcsTreeMaker::NXFBIN] )
{
...
void StMuFcsTreeMaker::DoTssaFit( TH1* h1_rawasymVphi[][StMuFcsTreeMaker::NXFBIN], TH1* h1_bluepoldata, TH1* h1_yellowpoldata, TH1* h1_AnResult[] )
{
  Double_t pi = TMath::Pi();
  //Double_t ebins[NENERGYBIN+1] = {0, 15, 20, 25, 30, 40, 55, 70, 100};  //Copied from above
  //h1_AnResult[0] = new TH1D("H1D_AnResultBlue","A_N vs. Energy;Energy;A_N",StMuFcsTreeMaker::NENERGYBIN,ebins);
  //h1_AnResult[1] = new TH1D("H1D_AnResultYellow","A_N vs. Energy;Energy;A_N",StMuFcsTreeMaker::NENERGYBIN,ebins);
...
void StMuFcsTreeMaker::DoPi0Fits(TH3* mH3F_invmass, TH1* hist_proj[] )
{
...
void StMuFcsTreeMaker::DoBgCorrectedAn(TH1* h1_invmass_xf[], TH1* h1_an_inc, TH1* h1_an_bg, TH1* h1_anresult )
{
...
void StMuFcsTreeMaker::PolData::Print() const
{
...
Int_t StMuFcsTreeMaker::ReadPolFile(const char* filename)
{
...
Double_t StMuFcsTreeMaker::pol4bg(Double_t* x, Double_t* par)
{
...
Double_t StMuFcsTreeMaker::skewgaus(Double_t*x, Double_t* par)
{
...
Double_t StMuFcsTreeMaker::pol4skewgaus(Double_t*x, Double_t* par)
{
  //if( x[0]<0.045 ){ TF1::RejectPoint(); return 0; }
...
void StMuFcsTreeMaker::PaintPhotonQaForDefense(TCanvas* canv, const char* savename)   const
{
  canv->Clear();
...
Int_t StMuFcsTreeMaker::DrawEpdProjection(TCanvas* canvas, const char* savename)
{
...
void StMuFcsTreeMaker::CheckInsideEpdTile(FcsPhotonCandidate* photon, Double_t projx, Double_t projy ) const
{
  //loop over all west epd tiles so that even if no hit recorded can use as a veto
  for(int i_pp=1; i_pp<=12; ++i_pp){     //Supersector runs [1,12]
    for( int i_tt=1; i_tt<=31; ++i_tt ){ //Tile number [1,31]
...
TPolyLine* StMuFcsTreeMaker::EpdTilePoly(short pp, short tt) const
{
  if( mEpdGeo==0 ){ return 0; }
  double x[5] = {0};
  double y[5] = {0};
...
//Makes the 2x2 
TPolyLine* StMuFcsTreeMaker::EpdCCWOuterCorner(short pp, short tt) const
{
...
TPolyLine* StMuFcsTreeMaker::EpdCWOuterCorner(short pp, short tt) const
{
  int newtt = 0;
  int newpp = 0;
...
TPolyLine* StMuFcsTreeMaker::EpdCCWInnerCorner(short pp, short tt) const
{
  int newtt = 0;
  int newpp = 0;
...
TPolyLine* StMuFcsTreeMaker::EpdCWInnerCorner(short pp, short tt) const
{
  int newtt = 0;
  int newpp = 0;
...
Int_t StMuFcsTreeMaker::GetColor(Double_t Value, Double_t MinVal, Double_t MaxVal)
{
...
void StMuFcsTreeMaker::DrawSelectEpdAdjTiles(TCanvas* canv) const
{
  if( mEpdGeo==0 ){ return; }
  if( canv==0 ) { return; }
...
void StMuFcsTreeMaker::DrawAllEpdAdjTiles(TCanvas* canv, Int_t tt) const
{
